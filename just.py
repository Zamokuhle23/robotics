...
def load_input():
    input_image = cv2.imread('sondo.jpg')
    #augment_image = cv2.imread('sondos.jpg')
    #vid = cv2.VideoCapture('vid.mp4')
    hT, wT, cT = input_image.shape

    input_image = cv2.resize(input_image, (wT, hT), interpolation=cv2.INTER_AREA)
    success, augment_image = vid.read()
    augment_image = cv2.resize(augment_image, (wT, hT))
