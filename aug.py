...
img1 = cv2.imread('sondo.jpg')  # This is the image identified for overlaying
win_name = 'Camera Matching'
MIN_MATCH = 10
#images = glob.glob('*.JPG')
images = cv2.VideoCapture('vid.mp4')# all the jpg images in the folder could be displayed
currentImage = 0  # the first image is selected
success, replaceImg = images.read()
...
#kp1, des1 = detector.detectAndCompute(imgTarget,None)
    ...
    if img1 is None:
        ...
        if len(good_matches) > MIN_MATCH:
            ...
            if mask.sum() > MIN_MATCH:
                ...
                rt, mk = cv2.threshold(cv2.cvtColor(distance, cv2.COLOR_BGR2GRAY), maskThreshold, 1,
